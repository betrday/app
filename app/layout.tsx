import type { Metadata } from "next";

import localFont from "next/font/local";
import "./globals.css";



const singel = localFont({
  src: [
    {
      path: "../fonts/Singel Regular.otf",
      weight: "400",
      style: "normal",
    },
    {
      path: "../fonts/Singel Semi Bold.otf",
      weight: "600",
      style: "normal",
    },
    {
      path: "../fonts/Singel Bold.otf",
      weight: "700",
      style: "normal",
    },
  ],
  variable: "--font-singel",
});

const sfProRounded = localFont({
  src: [
    {
      path: "../fonts/SF-Pro-Rounded-Thin.otf",
      weight: "100",
      style: "normal",
    },
    {
      path: "../fonts/SF-Pro-Rounded-Light.otf",
      weight: "300",
      style: "normal",
    },
    {
      path: "../fonts/SF-Pro-Rounded-Regular.otf",
      weight: "400",
      style: "normal",
    },
    {
      path: "../fonts/SF-Pro-Rounded-Medium.otf",
      weight: "500",
      style: "normal",
    },
    {
      path: "../fonts/SF-Pro-Rounded-Semibold.otf",
      weight: "600",
      style: "normal",
    },
    {
      path: "../fonts/SF-Pro-Rounded-Bold.otf",
      weight: "700",
      style: "normal",
    },
    {
      path: "../fonts/SF-Pro-Rounded-Heavy.otf",
      weight: "800",
      style: "normal",
    },
    {
      path: "../fonts/SF-Pro-Rounded-Black.otf",
      weight: "900",
      style: "normal",
    },
  ],
  variable: "--font-sf-pro-rounded",
});

export const metadata: Metadata = {
  title: "Betr Day",
  description: "",
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="en">
      <body
        suppressHydrationWarning
        className={`${singel.variable} ${sfProRounded.variable} antialiased`}
      >
        {children}
      </body>
    </html>
  );
}
